...
if($rowCount > 0){
	while($row = $query->fetch(PDO::FETCH_ASSOC)){
		$post_list[] = array(     "Client Name"=>$row["contact_person"],"Subject"=>$row["subject"],"Date"=>$row["appointment_date"],"Time"=>$row["appointment_time"],"Location"=>$row["location"],"Created At"=>$row["created_at"],"Status"=>$row["status"] );
		//$sno++;
	}
}
