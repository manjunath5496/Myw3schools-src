...
if($rowCount > 0){
	while($row = $query->fetch(PDO::FETCH_ASSOC)){
		$post_list[] = array( "Customer Name"=>$row["name"],"Contract Details"=>$row["description"],"Contract Value"=>$row["valuel"], "End"=>$row["endl"],"Start"=>$row["created_at"],"Status"=>$row["signature"] );
		//$sno++;
	}
}
