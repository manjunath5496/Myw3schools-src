...
echo '<meta http-equiv="refresh" content="0;url=index1.php?action=deleted" />';
	exit;
} 
...
<?php 
	...
	try {
			...
			while ($row = $stmt->fetch()){
				...
				echo '</br>';
				echo '<p style="font-size:15px;font-weight:bold;color:gray;">'.$row['postTitle'].'</p>';
				echo ' <hr>'  ;
				echo '<p style="color:gray;">'.date('jS M Y', strtotime($row['postDate'])).'</p>';
				?>
</br>
				<p>
					<a href="edit-post.php?id=<?php echo $row['postID'];?>" title="Edit Blog"><span  style='font-size:20px; color:#3A5580;'><i class='fa fa-edit' aria-hidden='true'></i></span></a> | 
					<a href="javascript:delpost('<?php echo $row['postID'];?>','<?php echo $row['postTitle'];?>')" title="Delete Blog"><span  style='font-size:20px; color:#ec5252;'><i class='fa fa-times' aria-hidden='true'></i></span></a>
				</p>
	...
	if($stmt->rowCount() == 0)
		{
	echo '<p style ="text-align:center; font-size:18px;"><a href="index.php" class ="pointer" title=""> No Records Found  </a></p>';
