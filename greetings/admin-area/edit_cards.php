    ...
    if ($card_title == "") {
        ...
        if ($card_file <> "") {
            ...
            if (in_array($ext, $allowed_ext)) {
                if (@move_uploaded_file($_FILES["card_file"]["tmp_name"], "../" . $tempPath)) {
                    ...
                    $msg = errorMessage("<strong>Error in Uploading Card! Please Try Again.</strong> <META HTTP-EQUIV='refresh' CONTENT='4;URL=edit_cards.php?edit=$card_id' />");
                }
            } else {
                $msg = errorMessage("<strong>File Type Not Supported.</strong> <META HTTP-EQUIV='refresh' CONTENT='4;URL=edit_cards.php?edit=$card_id' />");
            }
        }
        ...
        if ($msg == "") {
            ...
            try {
                ...
                $stmt->execute();
                if ($stmt->rowCount() > 0) {
                    $msg = successMessage("<strong>Card Updated Successfully.</strong> <META HTTP-EQUIV='refresh' CONTENT='4;URL=edit_cards.php?edit=$card_id' />");
                } else if ($stmt->rowCount() == 0) {
                    $msg = successMessage("<strong>No Changes Affected.</strong> <META HTTP-EQUIV='refresh' CONTENT='4;URL=edit_cards.php?edit=$card_id' />");
                } else {
                    $msg = errorMessage("<strong>Failed to Update Card.</strong> <META HTTP-EQUIV='refresh' CONTENT='4;URL=edit_cards.php?edit=$card_id' />");
                }
            } catch (Exception $ex) {
